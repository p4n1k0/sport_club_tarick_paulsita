...
export default class MatchController {
  ...
  async create(req: Request, res: Response) {
    ...
    if (homeTeam === awayTeam) return res.status(statusCodes.invalidFormat).json({ message: 'It is not possible to create a match with two equal teams' }, );
    if (!homeTeam || !awayTeam) return res.status(statusCodes.notFound).json({ message: 'There is no team with sich id!' }, );
